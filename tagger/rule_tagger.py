...
def is_present(text_list):
    ...
    for text in text_list:
        ...
        # else:
        #     dates = re.findall(r'(([0-9]), ([0-2][0-9]), ([3][0-1]))?(th, rd)?([\- ])?(Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, January, February, March, April, June, July, August, September, October, November, December)\b([\- ])?(([0-9]), ([0-2][0-9]), ([3][0-1]))?(th, rd)?', text)
        #     if len(dates) > 0:
        #         # Take first date
        #         # print(text)
        #         # print(dates)
        #         # date = dateparser.parse("".join(dates[0]))
        #         # print(date)
...
def filter_tweets(tweets, tweet_file, results_filename):
    new_filename = tweet_file.replace("tweets", "tweets_filtered")
...
if __name__ == "__main__":   
    ...
    sets = ['VLDset1', 'VLDset2', 'VLDset3']
    
    tweet_set = sys.argv[1]
    # tweet_file = sys.argv[2] #"tweets_20130630.csv"
